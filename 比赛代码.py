...
def build_model():
    ...
    other_params = {'learning_rate': 0.02,'n_estimators': 675,'max_depth': 3,'subsample': 0.6,
                  'reg_lambda': 3}
    cab = catboost.CatBoostRegressor(**other_params,logging_level=None)
    nn = KerasRegressor(build_fn=build_nn, nb_epoch=500, batch_size=32, verbose=2)
    return svr, line, lasso, ENet, KRR1, KRR2, lgbm, xgb, nn, cab
...
def main():
    ...
    # 创造特征 -- 使用高相关度特征相乘
#     all_data['V0_mul_V1'] = all_data['V0']*all_data['V1']
#     all_data['V0_mul_V8'] = all_data['V0']*all_data['V8']
#     all_data['V0_mul_V27'] = all_data['V0']*all_data['V27']
#     all_data['V0_mul_V31'] = all_data['V0']*all_data['V31']
#     all_data['V0_mul_V2'] = all_data['V0']*all_data['V2']

#     all_data['V1_mul_V8'] = all_data['V1']*all_data['V8']
#     all_data['V1_mul_V27'] = all_data['V1']*all_data['V27']
#     all_data['V1_mul_V31'] = all_data['V1']*all_data['V31']
#     all_data['V1_mul_V2'] = all_data['V1']*all_data['V2']
...
#     all_data["V0_mul_V27"] = np.exp(all_data["V0_mul_V27"])
    ...
    svr, line, lasso, ENet, KRR1, KRR2, lgbm, xgb, nn, cab = build_model()
...
#     X_scaled = pd.DataFrame(preprocessing.scale(X_train),columns = X_train.columns)
#     score = rmsle_cv(cab, X_train_head5, y_train)
#     print("CATB 得分: {:.4f} ({:.4f})\n" .format(score.mean(), score.std()))
#     cab.fit(X_train_head, y_train,logging_level=None)
    # =============================================================================
    head_feature_num = 18
    feat_scored_headnum = feature_scoring.sort_values('score', ascending=False).head(head_feature_num)['feature']
    X_train_head5 = X_train[X_train.columns[X_train.columns.isin(feat_scored_headnum)]]
    ...
    result.to_csv("result(averaged_test_0602_10).txt", index=False, header=False)
    print("Predict Done.")
    print(datetime.now())
